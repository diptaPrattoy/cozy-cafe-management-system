            ...
            else
            {
                if (connect.State == ConnectionState.Closed)
                {
                    try
                    {
                        ...
                        string selectProdID = "SELECT * FROM products2 WHERE prod_id = @prodID";
                        ...
                        using (SqlCommand selectPID = new SqlCommand(selectProdID, connect))
                        {
                            ...
                            {
                                string insertData = "INSERT INTO products2 (prod_id, prod_name, prod_type, " +
                ...
                if (check == DialogResult.Yes)
                {
                    if (connect.State != ConnectionState.Open)
                    {
                        try
                        {
                            ...
                            string updateData = "UPDATE products2 SET prod_name = @prodName" +
                ...
                if (check == DialogResult.Yes)
                {
                    if (connect.State != ConnectionState.Open)
                    {
                        try
                        {
                            ...
                            string updateData = "UPDATE products2 SET date_delete = @dateDelete WHERE prod_id = @prodID";
                            DateTime today = DateTime.Today;
                            ...
                            using (SqlCommand updateD = new SqlCommand(updateData, connect))
                            {
                                ...
                                MessageBox.Show("Removed successfully!", "Information Messsage", MessageBoxButtons.OK, MessageBoxIcon.Information);
                                displayData();
